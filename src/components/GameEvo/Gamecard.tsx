...
const GameCard: React.FC<GameCardProps> = ({ year, title, description = 'no description', imageUrl, isReversed, marquee }) => {
  return (
    <div className="bg-grey-100 shadow-lg rounded-lg overflow-hidden mb-10">
      ...
      <div className={`flex flex-col md:flex-row ${isReversed ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
        ...
        <div className="p-6 w-full md:w-1/2">
          <h2 className="text-2xl font-bold text-white">{title} ({year})</h2>
          <p className={`mt-4 text-white ${title === 'Pong' ? 'bg-[url("/path-to-your-gif.gif")] bg-cover' : ''}`}>
            {description}
          </p>
        </div>
      </div>
